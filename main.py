...
@app.get("/Travels")
def getTravels(session=Depends(get_session), currentUser: User = Depends(currentUser)) -> List[Travel]:
...
#region City
@app.get("/Cities")
def getTravels(session=Depends(get_session), currentUser: User = Depends(currentUser)) -> List[City]:
    cities = session.exec(select(City)).all()
    return cities

@app.post("/Cities/add", response_model=UserDefault)
def add_city(data: dict, session: Session = Depends(get_session)) -> CityDefault:
    city = City(**data)
    session.add(city)
    session.commit()
